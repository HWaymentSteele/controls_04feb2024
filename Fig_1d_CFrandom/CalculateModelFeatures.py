...
if __name__=='__main__':
    ...
    p.add_argument('--ref_struct', nargs='*', action='store', help='PDB reference structure.')
    ...
    if args.ref_struct is not None:
        for struct in args.ref_struct:
            ref_obj = md.load_pdb(struct)
            ref_dssp = calc_dssp(ref_obj)

            pdb_name = os.path.basename(struct).replace('.pdb','')

            df['rmsd_ref_%s' % pdb_name] = df.parallel_apply(lambda row: calc_rmsd(row['pdb'], ref_obj), axis=1)
            df['dist_dssp_%s' % pdb_name] = df.parallel_apply(lambda row: levenshtein(row['dssp_string'], ref_dssp), axis=1)
